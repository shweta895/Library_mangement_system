...
if choice == "HOME":
    ...
    if st.session_state['login'] == False:
        ...
        btn = st.button("Login")
        if btn:
            mydb = mysql.connector.connect(host="127.0.0.1", user="root", password="newrootpassword", database="lms", port = "8080" )
            c = mydb.cursor()
    ...
    if st.session_state['login']:
        ...
        if choice2 == "View All Books":
            ...
            def onclick(books):
                ...
                mydb2 = mysql.connector.connect(host="127.0.0.1", user="root", password="newrootpassword", database="lms", port = "8080" )
                c2 = mydb2.cursor()
    ...
    if st.session_state['llogin']:
        ...
        if choice2 == "View Issue Books":
            ...
            mydb = mysql.connector.connect(host="localhost", user="root", password="newrootpassword", database="lms", port = "8080")
            c = mydb.cursor()
            c.execute("select * from student")
            l = []
            for row in c:
